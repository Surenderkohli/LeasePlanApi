...
router.post("/upload-banner",bannerUpload.single('banner'),
...
router.get("/single-banner/:id", httpHandler(async (req, res) => {
  const { id } = req.params
...
router.delete("/delete/:id", httpHandler(async (req, res) => {
  const data = req.body
  const { id } = req.params
